...
1. **Org policy constraint**: CDN returned 403, so we proceeded to grant the `cloud-cdn-fill`
   ...
   (see `org-policy-planA-custom-constraint.md`).
...
which requires an org policy change (see `org-policy-planB-tag-based.md`). With a public
bucket, a Backend Bucket CDN works -- but the CDN layer becomes optional since direct GCS
URLs would also work.

**Design doc:** `design-cdn-backend-bucket.md`
...
Tested via `setup-cdn-hmac-test.sh` -- OIDC documents served successfully via CDN.
...
1. **HMAC key management** -- access key + secret stored in Terraform state (encrypted
   ...
   the key only grants read-only access to one bucket whose contents are already served
   publicly via CDN
...
**Design doc:** `design-cdn-hmac.md`
**Test scripts:** `setup-cdn-hmac-test.sh`, `cleanup-cdn-hmac-test.sh`
...
**Test scripts:** `setup-proxy-test.sh`, `cleanup-proxy-test.sh`
...
| **Key rotation** | None | Optional (HMAC key has read-only access to one bucket whose contents are already public via CDN) | None |
...
3. **Not yet explored** -- this option has not been prototyped or tested
